...
function loadGoogleTaskLists() {
  ...
  fetch("https://tasks.googleapis.com/tasks/v1/users/@me/lists", {
    ...
    .then((data) => {
      ...
      taskListDropdown.innerHTML = data.items
...
function sendTasksToGoogle() {
  ...
  const selectedAssignments = [
    ...document.querySelectorAll(".assignment-card.selected"),
  ];
  ...
  selectedAssignments.forEach((card) => {
    const assignmentTitle = card.querySelector(
      ".assignment-title a"
    ).textContent;
    const item = scrapedData.find(
      (task) => task.assignment === assignmentTitle
    );
    ...
    fetch(
      `https://tasks.googleapis.com/tasks/v1/lists/${selectedTaskList}/tasks`,
      {
        method: "POST",
        headers: {
